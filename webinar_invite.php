...
if (isset($_SERVER['HTTP_ORIGIN'])) {
	...
	require '../PHPMailer/src/SMTP.php';
		...
		$message .= "Webinar To Mails : ".$to_mail."<br>";
		...
		foreach($users as $val)
	    {
			...
			if($result){
        		...
        		$to_email = $uemail;
        		$message = $message;
        		$subject = "EXIMBNI Webinar Inviatation.";
        	    ...
        	    if($count>0)
        	    {
				    ...
				    $message = "You have received webinar invitation from $user_name";
				    
    				$ins_inbox= "INSERT INTO `inbox` (`user_id`, `country_id`, `title`, `notification`, `type`, `created`) VALUES ('$user_id', '$country', 'Webinar Invitation.', '$message', '1', '$todaysdate')";
            		$result_inbox = mysqli_query($conn,$ins_inbox);
            		
            		$ins_unotify= "INSERT INTO `user_notification` (`user_id`, `country_id`, `title`, `notification`, `type`, `created`) VALUES ('$user_id', '$country', 'Webinar Invitation.', '$message', '1', '$todaysdate')";
