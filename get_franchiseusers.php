...
if (isset($_SERVER['HTTP_ORIGIN'])) {
	...
	if($franchise_type =='CF'){
        ...
        $getusers="select id, name, user_type, mobile,email,business_name, business_address,country_id,state_id,isfranchise from users where id != '$user_id' AND country_id ='$f_country_id' group by id";
	    ...
	    if(mysqli_num_rows($res)>0){
	        ...
	        foreach($chapter_id as $val){
	            $getusers="select id, name, user_type, mobile,email,business_name,business_address,country_id,state_id,isfranchise from users where id='$val' and id != '$user_id' AND country_id ='$f_country_id' group by id";
	            $resusers = mysqli_query($conn,$getusers);
	            if($resusers){
	                while($gros=mysqli_fetch_assoc($resusers)){
	                    $outp[]=$gros;
	   ...
	   // $condition = "AND country_id ='$f_country_id' ";
        ...
        $getusers="select id, name, user_type, mobile,email,business_name, business_address,country_id,state_id,isfranchise from users where id != '$user_id' AND state_id ='$f_state_id' group by id";
	    $resusers = mysqli_query($conn,$getusers);
	    if($resusers){
	        while($gros=mysqli_fetch_assoc($resusers)){
	             $outp[]=$gros;
	    ...
	    if(mysqli_num_rows($res)>0){
	        ...
	        foreach($chapter_id as $val){
	            $getusers="select id, name, user_type, mobile,email,business_name, business_address,country_id,state_id,isfranchise from users where id='$val' and id != '$user_id' AND state_id ='$f_state_id' group by id";
	            $resusers = mysqli_query($conn,$getusers);
	            if($resusers){
	                while($gros=mysqli_fetch_assoc($resusers)){
	                    $outp[]=$gros;
